...
/**
 * The type Content availability health check.
 */
@SlingHealthCheck(name = "Content Availability Health Check", label = "Content Availability Health Check", description = "Checks the content availability.", tags = {
        "content", "availability" })
public class ContentAvailabilityHealthCheck extends AbstractRunmodeAwareHealthCheck {

    @Property(unbounded = PropertyUnbounded.ARRAY, label = "Paths to Check", description = "The list of paths to check, optionally with expected HTTP status responses. "
    ...
    protected void execute(String siteName, FormattingResultLog resultLog) {
        ...
        try (ResolverHolder holder = new ResolverHolder(resolverFactory)) {
            for (String p : paths) {
                lastPath = p;

                Resource resource = holder.getResolver().resolve(p);
                if (null == resource) {
                    failed++;
                    resultLog.warn("Path '{}' seems to be invalid or does not exist", p);
                } else {
                    resultLog.debug("Path '{}' exists, children count is '{}'", p,
                            Iterators.size(resource.getChildren().iterator()));
                }
                checked++;
            }
        } catch (Exception e) {
