...
public class ResultJsonSerializer {
    ...
    public String serialize(final Result overallResult, final List<HealthCheckExecutionResult> executionResults,
            final String jsonpCallback, boolean includeDebug) {
    ...
    private JSONObject getJsonForSimpleResult(final HealthCheckExecutionResult healthCheckResult, boolean includeDebug)
        ...
        for (ResultLog.Entry entry : healthCheckResult.getHealthCheckResult()) {
            ...
            jsonEntry.put("message", getAsJsonOrRawString(entry.getMessage()));
    ...
    private Object getAsJsonOrRawString(String value) {
        Object result = null;
        ...
        try {
            ...
            LOG.trace("Provided string seems to be not a valid json: '{}'", ex.getMessage());
        } finally {
            if (null == result) {
                result = value;
            }
